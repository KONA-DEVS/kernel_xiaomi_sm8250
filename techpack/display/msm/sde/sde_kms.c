...
static void sde_kms_add_pm_qos_irq_request(struct sde_kms *sde_kms)
{
...
static void sde_kms_pm_qos_remove_irq_request(struct sde_kms *sde_kms)
{
	if (!cpumask_empty(&sde_kms->irq_cpu_mask))
		pm_qos_remove_request(&sde_kms->pm_qos_irq_req);
}
		...
		const cpumask_t *mask)
{
	...
	// release vote from previous irq cpu mask
	if (sde_kms->irq_enabled)
		sde_kms_pm_qos_remove_irq_request(sde_kms);

	// save irq cpu mask
	sde_kms->irq_cpu_mask = *mask;

	// request vote with new irq cpu mask
	if (sde_kms->irq_enabled)
		sde_kms_add_pm_qos_irq_request(sde_kms);
}
