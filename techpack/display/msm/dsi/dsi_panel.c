	...
	u32 bl_lvl)
{
	...
	if (panel->mi_cfg.bl_is_big_endian) {
		if ((!use_count && bl_lvl) || (use_count && !bl_lvl)) {
			/* DSI_INFO("set backlight to %d\n", bl_lvl); */
			if (!use_count)
				use_count++;
			else
				use_count--;
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	DSI_INFO("panel backlight type:%d lvl:%d\n", bl->type, bl_lvl);
	...
	case DSI_BACKLIGHT_DCS:
		if (mi_cfg->fod_backlight_flag) {
			DSI_INFO("fod_backlight_flag set, skip set backlight\n");
		} else {
	...
	if ((mi_cfg->last_bl_level == 0 || (mi_cfg->skip_dimmingon == STATE_DIM_RESTORE)) && bl_lvl) {
...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	panel->mi_cfg.skip_dimmingon = STATE_DIM_RESTORE;
		...
		if (mi_cfg->fod_hbm_enabled && priv_info) {
			if (mi_cfg->hbm_51_ctrl_flag) {
				...
				if (cmds && count >= mi_cfg->fod_off_51_index) {
					...
					pr_err("0x%02X not need to update, please check 0x51 register index\n",
						tx_buf[0]);
				}
